...
export const isModalRoute = (route: RouteRecordRaw) => {
  return route.path.endsWith('.modal')
}
/**
 ...
 */
export const transformToModalRoute = (routes: readonly RouteRecordRaw[], inModalRoute: boolean = false) => {
  const prefix = 'modal-'
  return routes.map((aRoute) => {
    ...
    result.path = result.path.replaceAll(/\.modal/g, '')
    result.meta = {
      ...result.meta,
      modal: true,
    }
